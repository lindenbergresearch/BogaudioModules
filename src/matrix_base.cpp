...
void MatrixBaseModuleWidget::contextMenu(Menu *menu) {
    ...
    c->addItem(
        OptionMenuItem("Soft/saturated (better for audio)", [m]() { return m->_clippingMode == MatrixBaseModule::SOFT_CLIPPING; }, [m]() { m->_clippingMode = MatrixBaseModule::SOFT_CLIPPING; }));
...
void KnobMatrixModuleWidget::contextMenu(Menu *menu) {
    ...
    menu->addChild(
        new OptionMenuItem(
            ...
            [m, this]() {
                m->_indicatorKnobs = !m->_indicatorKnobs;
                this->redrawKnobs();
            }
        ));
    menu->addChild(
        new OptionMenuItem(
            ...
            [m, this]() {
                m->_unipolar = !m->_unipolar;
                m->updateParamMinimumValues();
                this->redrawKnobs();
            }
        ));
}
