...
float Additator::amplitudeNormalizationParam(int c) {
    return clamp(
        params[GAIN_PARAM].getValue() + ((maxAmplitudeNormalization - minAmplitudeNormalization) / 2.0f) * cvValue(c, inputs[GAIN_INPUT]), minAmplitudeNormalization, maxAmplitudeNormalization
    );
}
...
void Additator::processAlways(const ProcessArgs &args) {
    Phase phase = params[PHASE_PARAM].getValue() > 1.5f ? PHASE_COSINE : PHASE_SINE;
