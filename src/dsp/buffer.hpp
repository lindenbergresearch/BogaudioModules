...
struct AveragingBuffer {
    ...
    AveragingBuffer(
        int size, int framesToAverage
    ) :
        _size(size), _framesN(framesToAverage), _inverseFramesN(1.0 / (float) _framesN), _sums(new T[_size]{}), _averages(new T[_size]{}), _frames(new T[_size * _framesN]{})
        , _currentFrame(0), _resetsPerCommit(std::max(_size / 100, 10)), _currentReset(0) {
