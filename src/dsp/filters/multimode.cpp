...
template<>
void BiquadBank<MultimodeTypes::T, 4>::reset() {
    _biquads->reset();
}


template<>
void BiquadBank<MultimodeTypes::T, 4>::setN(int n, bool minDelay) {
    _biquads->setN(n, minDelay);
}


template<>
float BiquadBank<MultimodeTypes::T, 4>::next(float sample) {
...
template<>
void BiquadBank<MultimodeTypes::T, 8>::reset() {
    _biquads[0].reset();
    _biquads[1].reset();
}
...
template<>
void BiquadBank<MultimodeTypes::T, 8>::setN(int n, bool minDelay) {
    assert(n > 0 && n <= 8);
...
template<>
float BiquadBank<MultimodeTypes::T, 8>::next(float sample) {
...
template<>
void BiquadBank<MultimodeTypes::T, 16>::reset() {
    _biquads[0].reset();
    _biquads[1].reset();
    _biquads[2].reset();
    _biquads[3].reset();
...
template<>
void BiquadBank<MultimodeTypes::T, 16>::setN(int n, bool minDelay) {
    assert(n > 0 && n <= 16);
    for (int i = 0, nn = n / 4; i < nn; ++i) {
        _biquads[i].setN(4, false);
    }
...
template<>
float BiquadBank<MultimodeTypes::T, 16>::next(float sample) {
    sample = _biquads[0].next(sample);
...
template
struct BiquadBank<MultimodeTypes::T, 4>;
template
struct BiquadBank<MultimodeTypes::T, 8>;
template
struct BiquadBank<MultimodeTypes::T, 16>;
...
template<int N>
float MultimodeDesigner<N>::effectiveMinimumFrequency() {
...
template
struct MultimodeDesigner<4>;
template
struct MultimodeDesigner<8>;
template
struct MultimodeDesigner<16>;
...
template<int N>
float MultimodeBase<N>::next(float sample) {
    return _outGain * _biquads.next(sample);
}


template<int N>
void MultimodeBase<N>::reset() {
    _biquads.reset();
}


template
struct MultimodeBase<4>;
template
struct MultimodeBase<8>;
template
struct MultimodeBase<16>;
